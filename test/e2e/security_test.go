...
//
// It builds the CSR itself, unlike [fleet.issueControlLeaf], and deliberately:
// this is the --csr mode of `ca sign` — the one an agent's rotation uses, where
// the key is generated by whoever owns the identity and never travels — and
// with the fleet's own control leaf now coming from the no-CSR mode, this is
// the only place the suite drives it. The caller wants the key in hand to
// present as a client certificate, which is exactly the shape --csr serves.
