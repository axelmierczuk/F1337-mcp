...
| `TestInstallerAsksAndTheAnswersReachTheConfigAndTheFleet` | `install.sh` on a pseudo-terminal with nothing on its command line: it asks the posture and the listen address, downloads a release served from loopback, checks its checksum, writes the config, and prints the `fleetctl add` line — and the config it wrote starts a daemon that `fleetctl add` then finds serving (#100). |
| `TestInstallerOffersTheTailnetAddressFirstAndDefaultsToNothingUnsafe` | The address menu, against a host whose addresses are stubbed at the OS commands the script asks: the tailnet address first, the public one last, every entry labelled by what can reach it, no wildcard offered with mTLS off, and pressing return choosing the tailnet address rather than anything reachable from outside. |
| `TestInstallerRefusesAListenAddressTheAgentWouldNotStartOn` | `--no-mtls --listen 0.0.0.0:8722` is refused with the daemon's own reasoning and its three ways out, before the release server is touched at all — and the same address is not refused for an enrolling agent, which is what `fleetctl enroll mint` prints. |
| `TestInstallerEnrollsAndTheFleetReachesTheHost` | The mTLS posture end to end against a real control plane, for both token shapes: one that authorized an address, and one that authorized none — where the `--address` the installer passes is the only thing that keeps the fleet entry from being an agent nobody can dial. |
| `TestInstallerOnATerminalThatAnswersNothing` | #73's fixture pointed at the installer: it asks, waits, and when the input ends it refuses naming the flags that answer it — having downloaded nothing and installed nothing. |
| `TestInstallerWithoutATerminalSaysWhichFlagIsMissing` | Non-TTY stdin takes the non-interactive path rather than blocking, and each missing answer is an error naming the flag that supplies it. |
| `TestInstallerRefusesANameTheFleetCannotHold` | A name with a space in it is refused before the download, because the installer ends by printing a `fleetctl add` line built from it — and `fleetctl add` refuses the same name, so the rule is the fleet's rather than the installer's own. |
| `TestInstallerDoesNotClaimSuccessWhenRegistrationFails` | A `service install` that refuses fails the install: no "running", no `fleetctl add` line, and the binary and config left in place so that the command it tells the operator to re-run is one that can work. |
| `TestInstallerWithNoArgumentsInstallsTheBinaryAndSaysWhatIsLeft` | `curl \| sh` with nothing else still installs the binary, writes no config, and says what is left — an install that started guessing at a posture would be guessing about who may run commands on the host. |
| `TestInstallerRefusesAnArchiveTheChecksumDoesNotMatch` | The one security property this script has: an archive whose checksum is not the published one is not installed. |
