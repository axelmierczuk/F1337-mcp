...
//
// The CA issues every identity in the fleet and lives only in sandboxctl,
// never in sandboxd-mcp: nothing a model can reach through the MCP server
...
// sandboxd-mcp) — and each is restricted to its own extended key usage so a
