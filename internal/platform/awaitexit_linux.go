...
// awaitExit blocks until the process has exited, and leaves it unreaped. See
...
// pidfd-holding sweep would name exactly as stale a group id as the ordering
// this replaces. Linux 6.9's PIDFD_SIGNAL_PROCESS_GROUP would settle the
// question properly by signalling through the descriptor rather than through
// the number, but the agent ships to whatever kernel the host runs, and macOS
// has no equivalent at all; the ordering above is one mechanism for both.
func awaitExit(pid int) error {
	...
	for {
		...
		if err != nil {
			return fmt.Errorf("platform: waiting for pid %d to exit: %w", pid, err)
		}
		return nil
	}
}
