...
func TestEnrollmentMaterial(t *testing.T) {
	...
	}, fleetagent.EnrollmentMaterialForTest(cfg, configPath))
	...
	assert.Equal(t, []string{configPath, filepath.Join(dir, "agent.crt")},
		fleetagent.EnrollmentMaterialForTest(same, configPath))
}
...
func TestEnrollmentDirIsOurs(t *testing.T) {
	assert.True(t, fleetagent.EnrollmentDirIsOursForTest(agent.SystemConfigDir()))
	assert.True(t, fleetagent.EnrollmentDirIsOursForTest(agent.SystemConfigDir()+string(filepath.Separator)),
		"a trailing separator names the same directory")

	userDir, err := agent.UserConfigDir()
	require.NoError(t, err)
	assert.True(t, fleetagent.EnrollmentDirIsOursForTest(userDir))
	...
	for _, dir := range []string{
		...
		assert.False(t, fleetagent.EnrollmentDirIsOursForTest(dir),
