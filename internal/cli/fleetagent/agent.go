// Package fleetagent implements the fleet-agent CLI: enrollment, the
...
// Main runs fleet-agent and returns the process exit code.
...
func newEnrollCommand(out io.Writer) *cobra.Command {
	...
	cmd.Flags().StringVar(&f.token, "token", "", "single-use enrollment token from `fleetctl enroll mint`")
	cmd.Flags().StringVar(&f.fingerprint, "ca-fingerprint", "", "SHA-256 fingerprint of the fleet CA, from `fleetctl ca fingerprint`")
...
func runEnroll(out io.Writer, f enrollFlags) error {
	...
	if fingerprint == "" {
		...
		return fmt.Errorf("--ca-fingerprint is required; get it from `fleetctl ca fingerprint`")
	}
