...
func runServiceUninstall(out io.Writer) error {
	...
	p.Println("re-run `sandboxd-agent service install` to rejoin without enrolling again")
	return p.Err()
}
...
func newServiceControlCommand(out io.Writer, verb, short string) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(*cobra.Command, []string) error {
			...
			if !isInstalled(svc) {
				return fmt.Errorf("%w; run `sandboxd-agent service install` first", ErrNotInstalled)
			}
			switch verb {
			case "start":
				err = svc.Start()
...
func runServiceStatus(out io.Writer) error {
	...
	if errors.Is(statusErr, service.ErrNotInstalled) || status == service.StatusUnknown && statusErr != nil {
		...
		p.Printf("platform:   %s\n", service.Platform())
		p.Println("run `sandboxd-agent service install` to register it")
		return p.Err()
	}
...
func buildUnitParams(configPath, userName string, level Hardening) (UnitParams, *agent.Config, error) {
	...
	if err != nil {
		return UnitParams{}, nil, fmt.Errorf("%w\n\nRun `sandboxd-agent enroll` first, or pass --config with the path to an existing config", err)
	}
