// Package sandboxdmcp implements the sandboxd-mcp CLI: the MCP server an
...
func NewRootCommand(out io.Writer) *cobra.Command {
	root := &cobra.Command{
		...
		Long: "sandboxd-mcp is the MCP server an agent CLI launches over stdio. It owns\n" +
			"the fleet registry and the current selection, and turns MCP tool calls into\n" +
			"gRPC calls against sandboxd-agent instances.",
		SilenceUsage:      true,
