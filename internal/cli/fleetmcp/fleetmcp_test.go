...
func TestRoot_HelpNamesServeAndItsFlags(t *testing.T) {
	...
	require.Zero(t, fleetmcp.Main([]string{"--help"}, &out))
	assert.Contains(t, out.String(), "serve")

	out.Reset()
	require.Zero(t, fleetmcp.Main([]string{"serve", "--help"}, &out))
	help := out.String()
...
func TestServe_RejectsAnUnknownLogLevel(t *testing.T) {
	var out bytes.Buffer
	code := fleetmcp.Main([]string{"serve", "--log-level", "verbose", "--config-dir", t.TempDir()}, &out)
...
func TestUnknownCommand_FailsWithoutTouchingStdout(t *testing.T) {
	var out bytes.Buffer
	assert.Equal(t, 1, fleetmcp.Main([]string{"nonsense"}, &out))
...
func runServeWithClosedStdin(t *testing.T, args []string, out *bytes.Buffer) int {
	...
	go func() { done <- fleetmcp.Main(args, out) }()
